...
class _FilterDialogWidgetState extends State<FilterDialogWidget> {
  ...
  Future<void> _saveCategory() async {
    ...
    try {
      ...
      setState(() { _savedCode = code; _isSaving = false; });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      title: const Text('Search Filters', style: TextStyle(color: auroraMint, fontWeight: FontWeight.bold)),
      content: SizedBox(
        width: 420,
          ...
          child: Column(
            ...
            children: [
              SwitchListTile(
                ...
                title: const Text('Kids Mode', style: TextStyle(color: auroraMint)),
                value: _kidsMode,
                onChanged: (value) => setState(() => _kidsMode = value),
              ),
              const SizedBox(height: 12),
              _buildDropdown('Video Duration', _selectedDuration, ['any', 'short', 'medium', 'long'],
                  (v) { if (v != null) setState(() => _selectedDuration = v); }),
              const SizedBox(height: 12),
              _buildDropdown('Video Type', _selectedVideoType, ['Any', 'Live', 'Shorts', 'Videos'],
                  (v) { if (v != null) setState(() => _selectedVideoType = v); }),
              const SizedBox(height: 12),
              TextField(
                controller: widget.avoidWordsCtrl,
                style: const TextStyle(color: Colors.white),
                decoration: const InputDecoration(labelText: 'Words to Avoid', hintText: 'e.g. remix, clips, shorts'),
              ),
              const SizedBox(height: 12),
              TextField(
                controller: widget.advancedDescriptionCtrl,
                style: const TextStyle(color: Colors.white),
                decoration: const InputDecoration(labelText: 'Advanced Description', hintText: 'Optional extra detail'),
              ),
              const SizedBox(height: 20),
              const Divider(color: auroraDeep),
              const SizedBox(height: 8),
              const Text('Save & Share', style: TextStyle(color: auroraGlow, fontWeight: FontWeight.w600)),
              const SizedBox(height: 8),
              TextField(
                controller: _categoryNameCtrl,
                style: const TextStyle(color: Colors.white),
                decoration: const InputDecoration(labelText: 'Category Name', hintText: 'e.g. Chill Lo-Fi Evenings'),
              ...
              if (_savedCode != null)
                Container(
                  ...
                  decoration: BoxDecoration(color: auroraDeep, borderRadius: BorderRadius.circular(10)),
                  child: Row(
                    children: [
                      const Icon(Icons.check_circle, color: Colors.greenAccent, size: 18),
                      const SizedBox(width: 8),
                      Text(_savedCode!, style: const TextStyle(color: auroraGlow, fontSize: 20, fontWeight: FontWeight.bold, letterSpacing: 4)),
                      const Spacer(),
                      IconButton(
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: () { widget.onApply(_kidsMode, _selectedDuration, _selectedVideoType); Navigator.pop(context); },
